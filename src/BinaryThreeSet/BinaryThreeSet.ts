...
export default class BinaryThreeSet<T> implements BinaryThree<T> {
  ...
  protected root: CanUndef<ThreeNode<T>>;
  ...
  get getRoot(): CanUndef<ThreeNode<T>> {
    return this.root
  }
  ...
  protected *prefix(node: CanUndef<ThreeNode<T>>): IterableIterator<T> {
    if (node) {
      yield node.data;
  ...
  protected *postfix(node: CanUndef<ThreeNode<T>>): IterableIterator<T> {
    if (node) {
      if (node.left) {
  ...
  protected *infex(node: CanUndef<ThreeNode<T>>): IterableIterator<T> {
    if (node) {
      if (node.left) {
  ...
  protected deleteNode(target: CanUndef<ThreeNode<T>>, parent: CanUndef<ThreeNode<T>>, side: boolean) {
  ...
  protected getSuccessor(target: ThreeNode<T>) {
